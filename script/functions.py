...
def is_regular_sequence(seq):
        Vérifie si une séquence suit un pattern régulier.
    """
    if len(seq) < 2:
        return False
...
def has_repetition(seq, max_length=2):
    """
    Vérifie s'il y a des répétitions de sous-séquences.
    """
    positions = [s[0] for s in seq]
...
def has_symmetry(seq):
    """
    Vérifie s'il y a une symétrie.
    """
    positions = [s[0] for s in seq]
...
def matches_regular_pattern(seq, regular_sequences):
    """
    Vérifie si la séquence correspond à un début de séquence régulière.
    """
    positions = [s[0] for s in seq]
    
    for reg_seq in regular_sequences:
        reg_positions = [s[0] for s in reg_seq]
        
        # Tester toutes les rotations possibles
        for rotation in range(NUM_OF_DOT):
            rotated = [(p + rotation) % NUM_OF_DOT for p in reg_positions]
            
            # Sens horaire
            if len(positions) <= len(rotated):
                if positions == rotated[:len(positions)]:
                    return True
            
            # Sens antihoraire
            reversed_rotated = rotated[::-1]
            if len(positions) <= len(reversed_rotated):
                if positions == reversed_rotated[:len(positions)]:
                    return True
...
def is_valid_irregular_sequence(seq, regular_sequences):
    ...
    if matches_regular_pattern(seq, regular_sequences):
        return False
...
def init_exp():
    """
    Initialisation de l'expérience avec les 768 séquences irrégulières.
    """
    # Les 8 séquences régulières de base (longueur 3 pour générer le pool)
    regular_sequences_base = [
        [(i, 1) for i in range(3)] for _ in range(8)
    ]  # À remplacer par vos vraies séquences de C_sequences_ez et C_sequences
...
def is_regular_sequence(seq): #    Vérifie si une séquence est régulière (pattern constant).
    ...
    # Calculer les différences entre positions consécutives (modulo 8)
    differences = [(seq[i+1][0] - seq[i][0]) % NUM_OF_DOT for i in range(len(seq) - 1)]
    
    # Vérifier si toutes les différences sont identiques
    return len(set(differences)) == 1

def has_repetition(seq, max_length=3): #    Vérifie s'il y a des répétitions de sous-séquences.
    positions = [s[0] for s in seq]
...
def has_consecutive_repetition(seq):
   #  Vérifie s'il y a répétition immédiate d'une même position.
    positions = [s[0] for s in seq]
    for i in range(len(positions) - 1):
        if positions[i] == positions[i+1]:
            return True
...
def has_symmetry(seq):#    Vérifie s'il y a une symétrie dans la séquence.
    ...
    # Symétrie miroir simple
    if positions == positions[::-1]:
        return True
    
    # Symétrie avec centre (pour octogone)
    # Deux points sont symétriques s'ils sont à l'opposé (différence de 4 positions)
    center_symmetric = True
    for i in range(n // 2):
        if (positions[i] + 4) % NUM_OF_DOT != positions[n - 1 - i]:
            center_symmetric = False
            break
    
    if center_symmetric and n > 2:
        return True
...
def is_valid_irregular_sequence(seq, regular_sequences):
    ...
    if has_consecutive_repetition(seq):
        return False
...
def create_random_irregular_seq(length, regular_sequences, max_attempts=1000):
    for attempt in range(max_attempts):
        # Générer une séquence aléatoire
        seq = []
        current_pos = rd.randint(0, NUM_OF_DOT - 1)
        seq.append((current_pos, 1))  # 1 pour sens horaire par défaut
        
        for _ in range(length - 1):
            # Choisir la prochaine position (éviter répétition immédiate)
            available_positions = [p for p in range(NUM_OF_DOT) if p != current_pos]
            
            # Biaiser vers des sauts variés (pas toujours +1 ou +2)
            next_pos = rd.choice(available_positions)
            
            # Alterner direction aléatoirement
            direction = rd.choice([1, -1])
            
            seq.append((next_pos, direction))
    ...
    # Si on n'a pas réussi après max_attempts
    print(f"Warning: Could not generate valid irregular sequence after {max_attempts} attempts")
    return None

def generate_all_irregular_sequences(num_sequences, length, regular_sequences):
    irregular_sequences = []
    
    for i in range(num_sequences):
        seq = create_random_irregular_seq(length, regular_sequences)
        if seq is not None:
            irregular_sequences.append(seq)
        else:
            print(f"Failed to generate irregular sequence {i+1}/{num_sequences}")
    
    return irregular_sequences
